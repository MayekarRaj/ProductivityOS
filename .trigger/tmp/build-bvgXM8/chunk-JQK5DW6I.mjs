...
__export(schema_exports, {
  days: () => days,
  daysRelations: () => daysRelations,
  events: () => events,
  eventsRelations: () => eventsRelations,
  habitLogs: () => habitLogs,
  habitLogsRelations: () => habitLogsRelations,
  habits: () => habits,
  habitsRelations: () => habitsRelations,
...
var tasks = pgTable("tasks", {
  ...
  area: text("area"),
  // nullable — defaults to day's homeBase in the UI
...
var inboxItems = pgTable("inbox_items", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  text: text("text").notNull(),
  source: text("source").notNull().default("web"),
...
var telegramConversations = pgTable("telegram_conversations", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var notifications = pgTable("notifications", {
  ...
  // 'morning_briefing' | 'event_reminder' | 'habit_reminder' | 'weekly_review'
...
var daysRelations = relations(days, ({ one, many }) => ({
...
var habitsRelations = relations(habits, ({ one, many }) => ({
  user: one(users, { fields: [habits.userId], references: [users.id] }),
  logs: many(habitLogs)
}));
var habitLogsRelations = relations(habitLogs, ({ one }) => ({
...
// trigger/utils/db.ts
var client = src_default(process.env.DATABASE_URL, { max: 5 });
var db = drizzle(client, { schema: schema_exports });

// trigger/utils/telegram.ts
init_esm();
var getBaseUrl = () => `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}`;
async function sendMessage(chatId, text2, parseMode = void 0) {
...
// trigger/utils/user.ts
init_esm();
var DEFAULT_USER_ID = "00000000-0000-0000-0000-000000000001";
async function getTelegramChatId() {
  const [user] = await db.select({ telegramChatId: users.telegramChatId }).from(users).where(eq(users.id, DEFAULT_USER_ID));
...
//# sourceMappingURL=chunk-JQK5DW6I.mjs.map
